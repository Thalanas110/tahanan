// POST /functions/v1/join-cof  { code: string }
// Looks up a COF by invite code via the join_cof_by_code
// security-definer RPC (needed because the caller isn't a member yet, so RLS
// alone would hide the cof row) and adds the caller as its second member.
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { data, error } = await supabase.rpc('join_cof_by_code', {
      ...
      const message = error.message.includes('NOT_AUTHENTICATED')
        ? 'Not authenticated'
          : error.message.includes('ALREADY_PAIRED')
            ? 'You are already part of a COF'
            : error.message.includes('INVALID_CODE')
              ? 'That invite code was not found'
              : error.message.includes('COF_FULL')
                ? 'That COF already has two members'
                : error.message;
      return errorResponse(message, 400);
    }

    return jsonResponse({ cof: data?.[0] ?? null });
  } catch (err) {
